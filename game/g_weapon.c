...
static void fire_leadshort (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int kick, int te_impact, int hspread, int vspread, int mod)
{
	trace_t		tr;
	vec3_t		dir;
	vec3_t		forward, right, up;
...
void fire_bulletshort (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int kick, int hspread, int vspread, int mod)
{
	fire_leadshort (self, start, aimdir, damage, kick, TE_GUNSHOT, hspread, vspread, mod);
}
...
void freon_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	int		mod;
	int		tempevent;
...
void fire_freon (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, int effect, qboolean hyper, int color)
{
	edict_t	*bolt;
	trace_t	tr;
	vec3_t	forward, right, up;
	...
	VectorMA (dir, crandom() * 0.04, up, dir);
	...
	//bolt->s.modelindex = gi.modelindex ("models/objects/laser/tris.md2");
	bolt->s.modelindex = gi.modelindex ("sprites/freonparticle.sp2");
	bolt->style = color;
...
void bfg_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (other->takedamage)
		T_Damage (other, self, self->owner, self->velocity, self->s.origin, plane->normal, bfg_rdamage->value, 0, 0, MOD_BFG_BLAST);
	T_RadiusDamage(self, self->owner, bfg_rdamage->value, other, 100, MOD_BFG_BLAST, -0.5);
