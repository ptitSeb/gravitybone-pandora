...
void SP_func_door (edict_t *ent)
{
	...
	else if (ent->sounds == 2)
	{
		...
		ent->moveinfo.sound_middle = gi.soundindex  ("plats/p_loop.wav");
		ent->moveinfo.sound_end = gi.soundindex  ("plats/p_end.wav");
	}
	else if (ent->sounds != 1)
	{
...
void train_wait (edict_t *self)
{
	...
	{
		gi.sound(self, CHAN_ITEM, gi.soundindex("plats/p_end.wav"), 1.0, ATTN_NORM, 0);
...
void train_use (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	else
	{
		if (!strcmp(self->classname, "func_train"))
		{
			...
			//gi.sound(self, CHAN_ITEM, gi.soundindex("plats/p_start.wav"), 0.7, ATTN_NORM, 0);
...
//		return;
	...
	if (st.noise)
		...
		self->moveinfo.sound_middle = gi.soundindex  ("plats/p_loop.wav");
...
void door_secret_use (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	//added sound
	if (self->moveinfo.sound_start)
		gi.sound (self, CHAN_NO_PHS_ADD+CHAN_VOICE, self->moveinfo.sound_start, 1, ATTN_NONE, 0);
	if (self->moveinfo.sound_middle)
		self->s.sound = self->moveinfo.sound_middle;
		//gi.sound (self, CHAN_NO_PHS_ADD+CHAN_VOICE, self->moveinfo.sound_middle, 1, ATTN_NONE, 0);
...
void door_secret_move1 (edict_t *self)
{
	...
	if (self->moveinfo.sound_end)
		gi.sound (self, CHAN_NO_PHS_ADD+CHAN_VOICE, self->moveinfo.sound_end, 1, ATTN_NONE, 0);
}
...
void box_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if(tpp_auto->value && (!cl_3dcam->value || deathmatch->value || coop->value)
...
void lava_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	int cont;
	...
	cont = gi.pointcontents (self->s.origin);
	if (cont & MASK_WATER)
	{
		gi.sound(self, CHAN_ITEM, gi.soundindex("world/tinysplash.wav"), 1, ATTN_NORM, 0);
	}
	else// if (plane && other)
	{
		//if(Q_stricmp(other->classname,"worldspawn") == 0)
			//gi.sound(self, CHAN_ITEM, gi.soundindex("world/sizzle.wav"), 1, ATTN_NORM, 0);
	}
...
void lavashoot (edict_t *self)
{
	...
	//VectorCopy(origin,self->maxs);

	//gi.dprintf("%f %f %f\n", self->s.origin[0],self->s.origin[1],self->s.origin[2]);
	//gi.dprintf("%f %f %f\n", self->maxs[0],self->maxs[1],self->maxs[2]);


	origin[0] = self->mins[0] + (crandom() * (self->maxs[0] - self->mins[0]));
	origin[1] = self->mins[1] + (crandom() * (self->maxs[1] - self->mins[1]));
	...
	gib->s.origin[0] = origin[0];
	gib->s.origin[1] = origin[1];
	gib->s.origin[2] = self->maxs[2];// + crandom() * 2;
	...
	//VelocityForDamage (512, vd);
	//VectorMA (self->velocity, vscale, vd, gib->velocity);

	//ClipGibVelocity (gib);


				gib->velocity[0] = (-128 + random()*128);
				gib->velocity[1] = (-128 + random()*128);
				gib->velocity[2] = (160 + random()*384);

	//gib->avelocity[0] = random()*200;
	//gib->avelocity[1] = random()*200;
	//gib->avelocity[2] = random()*200;

	gib->think = G_FreeEdict; //Knightmare- gib fade, was G_FreeEdict
	gib->nextthink = level.time + 5;

	gib->s.sound = gi.soundindex("world/lavaball.wav");

	gib->clipmask = MASK_SHOT;
	gib->solid = SOLID_BBOX;

	gib->touch = lava_touch;
...
void lavashoot_think (edict_t *self)
{
	lavashoot(self);


	self->think = lavashoot_think;
	self->nextthink = level.time + 1.5 + crandom()*1.0;
}




void lavashoot_use (edict_t *self)
{
	lavashoot(self);
	lavashoot(self);
	lavashoot(self);
...
void SP_func_lavashooter (edict_t *self)
{
	...
	//gi.dprintf("org %f %f %f\n", self->s.origin[0],self->s.origin[1],self->s.origin[2]);
	//gi.dprintf("max %f %f %f\n", self->maxs[0],self->maxs[1],self->maxs[2]);
...
void usebutton_action(edict_t *self, edict_t *other)
{
...
void usebutton_use(edict_t *self, edict_t *other, edict_t *activator)
{
	if(self->spawnflags & 2)
	{
		self->spawnflags &= ~2;
...
void SP_func_usebutton (edict_t *ent)
{
	ent->movetype = MOVETYPE_STOP;
