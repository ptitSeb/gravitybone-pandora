...
void target_photocounter_use(edict_t *self, edict_t *other, edict_t *activator)
{
	edict_t	*player;
	...
	player->client->ps.stats[STAT_MAXPHOTOS] = self->mass;
	player->client->ps.stats[STAT_PHOTOCOUNT] = self->mass - self->count;

	gi.sound (activator, CHAN_AUTO, gi.soundindex ("world/chaching.wav"), 0.5, ATTN_NORM, 0);



	//gi.dprintf("bla bla %d\n",player->client->ps.stats[STAT_PHOTOCOUNT]);


	if (self->count)
	{
		//increase the counter display.
...
void SP_target_photocounter (edict_t *self)
{
	...
	player->client->ps.stats[STAT_MAXPHOTOS] = self->count;

	//gi.dprintf("ZZZ bla bla %d\n",player->client->ps.stats[STAT_MAXPHOTOS]);

	self->use = target_photocounter_use;
}