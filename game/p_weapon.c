...
void ChangeWeapon (edict_t *ent)
{
	...
	if(!ent->client->chasetoggle)
	{
		...
		//ent->client->ps.gunindex = gi.modelindex(ent->client->pers.weapon->view_model);
	//client->ps.gunindex2 = gi.modelindex("models/weapons/hammer/tris.md2");
...
void Weapon_Generic2 (edict_t *ent, int FRAME_ACTIVATE_LAST, int FRAME_FIRE_LAST, int FRAME_IDLE_LAST, int FRAME_DEACTIVATE_LAST, int *pause_frames, int *fire_frames, void (*fire)(edict_t *ent, qboolean altfire))
{
	...
	if (ent->client->weaponstate == WEAPON_ACTIVATING)
	{
		...
		ent->client->ps.stats[STAT_WEAPBOX] = 100;
	...
	if ((ent->client->newweapon) && (ent->client->weaponstate != WEAPON_FIRING))
	{
		...
		gi.sound(ent, CHAN_ITEM, gi.soundindex("weapons/weapswitch.wav"), 1, ATTN_NORM, 0);
	...
	if (ent->client->weaponstate == WEAPON_FIRING)
	{
		...
		if ((strcmp(weap, "weapon_shotgun") == 0) && (ent->client->ps.gunframe == 4))
			gi.sound(ent, CHAN_ITEM, gi.soundindex("weapons/whoosh.wav"), 1, ATTN_NORM, 0);
...
//	fire_grenade (ent, start, forward, damage, grenade_speed->value, 25, radius);
...
void Blaster_Fire (edict_t *ent, vec3_t g_offset, int damage, qboolean hyper, int effect, int color)
{
	...
	VectorSet(offset, 12, 0.5, ent->viewheight+7);
...
void Freon_Fire (edict_t *ent, vec3_t g_offset, int damage, qboolean hyper, int effect, int color)
{
	vec3_t	forward, right;
	vec3_t	start;
	vec3_t	offset;
	...
	//VectorSet(offset, 0, 1.4, ent->viewheight-0.9);
	VectorSet(offset, 0, 0.45, ent->viewheight-0.7);
	...
	fire_freon (ent, start, forward, 1, 160, 0, false, color);
}
...
/*
	...
	Freon_Fire (ent, vec3_origin, damage, false, effect, color);
...
void Weapon_Blaster (edict_t *ent)
{
	...
	*/
	...
	static int	pause_frames[]	= {7};
	static int	fire_frames[]	= {4,0};




	Weapon_Generic (ent, 3, 6, 8, 11, pause_frames, fire_frames, Weapon_Blaster_Fire);
...
void Weapon_Shotgun (edict_t *ent)
{
	...
	*/
	...
	Weapon_Generic (ent, 3, 10, 12, 15, pause_frames, fire_frames, weapon_shotgun_fire);
}
...
void weapon_supershotgun_fire (edict_t *ent, qboolean altfire)
{
	...
	VectorSet(offset, 0, 4,  ent->viewheight-4);
	P_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);
...
void Weapon_SuperShotgun (edict_t *ent)
{
	...
	*/
	...
	static int	fire_frames[]	= {3,4,5,6,7,8,9,10, 0};

	Weapon_Generic (ent, 2, 13, 18, 20, pause_frames, fire_frames, weapon_supershotgun_fire);
}
...
void Weapon_Camera(edict_t *ent)
{
	...
	if (ent->client->weaponstate == WEAPON_READY)
	{
		if ( ((ent->client->latched_buttons|ent->client->buttons) & BUTTONS_ATTACK) && (ent->client->cardraising==0))
		{
			trace_t     tr;
			...
			ent->client->cardraising=1;
			gi.sound(ent, CHAN_ITEM, gi.soundindex("weapons/camera.wav"), 1, ATTN_NORM, 0);

			ent->client->kick_angles[0] = 0.5;



			//camera picture logic


			VectorMA(start, 8192, forward, end);

			tr = gi.trace (start, NULL, NULL, end, &g_edicts[1], MASK_SHOT);

			if (tr.ent && tr.ent->classname)
			{
				if(!stricmp(tr.ent->classname,"item_photoitem"))
				{
					photoitem_action(tr.ent,ent);
					//gi.dprintf("wheeee\n");
				}
			}
		...
		else if ( !((ent->client->latched_buttons|ent->client->buttons) & BUTTONS_ATTACK))
		{
			ent->client->cardraising=0;
		}
...
{
	int LASTACTIVATEFRAME = 4;
	int FIRSTIDLEFRAME = 5;
	int HOLDFRAME = 8;
	...
	if (ent->client->weaponstate == WEAPON_ACTIVATING)
	{
		...
		if (ent->client->ps.gunframe == LASTACTIVATEFRAME)
		{
			ent->client->weaponstate = WEAPON_READY;
			ent->client->ps.gunframe = FIRSTIDLEFRAME;
			return;
		}
	...
	if (ent->client->weaponstate == WEAPON_READY)
	{
		if ( ((ent->client->latched_buttons|ent->client->buttons) & BUTTONS_ATTACK) )
		{
			if (ent->client->cardraising == 0)
			{
				if (ent->client->ps.gunframe == HOLDFRAME)
				{
					//lower the card.
					ent->client->cardraising = 2;
				}
				else if (ent->client->ps.gunframe == FIRSTIDLEFRAME)
				{
					//raise the card.
					ent->client->cardraising = 1;
				}
				else if (ent->client->cardraising == 1)
					ent->client->cardraising = 2;
				else
					ent->client->cardraising = 1;

				if (ent->client->cardraising == 1)
				{
					gi.sound(ent, CHAN_ITEM, gi.soundindex("weapons/card_up.wav"), 0.2, ATTN_NORM, 0);
				}
				else if (ent->client->cardraising == 2)
				{
					gi.sound(ent, CHAN_ITEM, gi.soundindex("weapons/card_down.wav"), 0.1, ATTN_NORM, 0);
				}
			}
		}
		...
		else if (ent->client->cardraising == 2)
		{
			...
			if ((ent->client->ps.gunframe == 10) && (ent->client->ps.gunframe != FIRSTIDLEFRAME))
			{
				ent->client->cardraising = 0;
				ent->client->ps.gunframe = FIRSTIDLEFRAME;
			}
...
void Weapon_BFG (edict_t *ent)
{
	...
	if (ent->client->weaponstate == WEAPON_READY)
	{
		...
		else
		{
			if (ent->client->cardraising < 0)
			{
				ent->client->cardraising = 10+random() *30;
				ent->client->ps.gunframe= 5;
			}
			else
				ent->client->cardraising--;
		}
	}


	/*
	static int	pause_frames[]	= {1, 0};
	static int	fire_frames[]	= {1, 0};

	Weapon_Generic (ent, 5, 6, 7, 8, pause_frames, fire_frames, weapon_bfg_fire);
	*/
}
