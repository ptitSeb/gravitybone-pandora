	...
	if (waterlevel && (current_player->watertype&(CONTENTS_LAVA|CONTENTS_SLIME)) )
	{
		if (current_player->watertype & CONTENTS_LAVA)
		{
			...
			if (envirosuit)	// take 1/3 damage with envirosuit
				...
				T_Damage (current_player, world, world, vec3_origin, current_player->s.origin, vec3_origin, 60, 0, 0, MOD_LAVA);
...
void G_SetClientSound (edict_t *ent)
{
	...
	else if ((strcmp(weap, "weapon_blaster") == 0) && (ent->client->buttons & BUTTONS_ATTACK) && (ent->client->ps.gunframe > 3) && (ent->client->ps.gunframe < 9))
		ent->s.sound = gi.soundindex("weapons/spray.wav");
	else if ((strcmp(weap, "weapon_supershotgun") == 0) && (ent->client->buttons & BUTTONS_ATTACK) && (ent->client->ps.gunframe > 2) && (ent->client->ps.gunframe < 19))
		ent->s.sound = gi.soundindex("weapons/drill.wav");
