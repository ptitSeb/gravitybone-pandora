...
void guard_think(edict_t *self)
{
	...
	if (player)
	{
		...
		//clamp to 90 degree arc.
			...
			//vectoangles(self->s.old_origin,oldangles);
			
			
			
			//Com_Printf ("clview%f / yaw %f / ymin %f / ymax %f\n", cl.viewangles[YAW], yaw,ymin,ymax);
    /*
            if (playervec[YAW]  > ymax)
                    playervec[YAW] = ymax - playervec[YAW];
            else if (playervec[YAW]  < ymin)
                    playervec[YAW] = ymin - playervec[YAW];*/
...
void SP_monster_guard(edict_t *self)
{
	...
	//zbody->s.modelindex = gi.modelindex ("models/monsters/guard/body.md2");
