...
void camera_off (edict_t *ent)
{
	...
	// Kngihtmare- check if client-side chasecam was previously on
	if (ent->style & 2)
	{
		gi.cvar_forceset("cl_3dcam", "1");
		ent->style &= ~2;
	}
}
...
void use_camera (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	if (!deathmatch->value && !coop->value && cl_3dcam->value)
	{
		gi.cvar_forceset("cl_3dcam", "0");
		activator->style |= 2;
	}
