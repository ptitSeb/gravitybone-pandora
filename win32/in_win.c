...
enum _ControlList
{
	AxisNada = 0, AxisForward, AxisLook, AxisSide, AxisTurn, AxisUp, ztrigger
};
...
void IN_Init (void)
{
	...
	joy_advaxisx			= Cvar_Get ("joy_advaxisx",				"3",		CVAR_ARCHIVE); //strafe 
	joy_advaxisy			= Cvar_Get ("joy_advaxisy",				"1",		CVAR_ARCHIVE);
	joy_advaxisz			= Cvar_Get ("joy_advaxisz",				"6",		CVAR_ARCHIVE);
	joy_advaxisr			= Cvar_Get ("joy_advaxisr",				"2",		CVAR_ARCHIVE); //look up/down
	joy_advaxisu			= Cvar_Get ("joy_advaxisu",				"4",		CVAR_ARCHIVE);
	joy_advaxisv			= Cvar_Get ("joy_advaxisv",				"0",		CVAR_ARCHIVE);
	joy_forwardthreshold	= Cvar_Get ("joy_forwardthreshold",		"0.3",		0);
	joy_sidethreshold		= Cvar_Get ("joy_sidethreshold",		"0.3",		0);
	joy_upthreshold  		= Cvar_Get ("joy_upthreshold",			"0.3",		0);
	joy_pitchthreshold		= Cvar_Get ("joy_pitchthreshold",		"0.3",		0);
	joy_yawthreshold		= Cvar_Get ("joy_yawthreshold",			"0.3",		0);
	joy_forwardsensitivity	= Cvar_Get ("joy_forwardsensitivity",	"-1",		0);
	joy_sidesensitivity		= Cvar_Get ("joy_sidesensitivity",		"1",		0);
	joy_upsensitivity		= Cvar_Get ("joy_upsensitivity",		"-1",		0);
	joy_pitchsensitivity	= Cvar_Get ("joy_pitchsensitivity",		"0.3",		0);
	joy_yawsensitivity		= Cvar_Get ("joy_yawsensitivity",		"-0.85",		0);
		...
		case AxisLook:
			//if (mlooking)
			{
				if (fabs(fAxisValue) > joy_pitchthreshold->value)
				{
					...
					//bc joystick pitch is modified by mouse invert setting.
					float pitchsensitivity = joy_pitchsensitivity->value;
					...
					{
						if (autosensitivity->value) // Knightmare added
							cl.viewangles[PITCH] += (fAxisValue * pitchsensitivity * (cl.refdef.fov_x/90.0)) * aspeed * cl_pitchspeed->value;
						else
							cl.viewangles[PITCH] += (fAxisValue * pitchsensitivity) * aspeed * cl_pitchspeed->value;
					}
					else
					{
						if (autosensitivity->value) // Knightmare added
							cl.viewangles[PITCH] += (fAxisValue * pitchsensitivity * (cl.refdef.fov_x/90.0)) * speed * 180.0;
						else
							cl.viewangles[PITCH] += (fAxisValue * pitchsensitivity) * speed * 180.0;
					}
				}
			}
			break;
