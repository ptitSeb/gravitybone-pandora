...
void R_DrawParticleArrays (void)
{
	...
	RB_DrawArrays (currentParticleState.polymode);

	RB_DrawMeshTris (currentParticleState.polymode, 1);
...
void R_CheckParticleRenderState (particleRenderState_t *check, int numVerts, int numIndex)
{
	...
	// check for overflow
	if ((rb_vertex + numVerts >= MAX_VERTICES)
		|| (rb_index + numIndex >= MAX_INDICES))
		R_DrawParticleArrays();
...
{	
	...
	if (!r_particletextures[type])
		r_particletextures[type] = r_notexture;
	return r_particletextures[type]->texnum;
}
...
void R_RenderParticle (particle_t *p)
{
	...
	if ((p->flags & PART_BEAM) || (p->flags & PART_DIRECTION) || (p->flags & PART_ANGLED))
		numVerts = numIndex = 4;
	else if (p->flags & PART_SPARK)
	{	numVerts = 9;	numIndex = 24;	}
	else if (p->flags & PART_LIGHTNING) {
		VectorSubtract(p->origin, p->angle, move);
		len = VectorNormalize(move);
		numVerts = numIndex = (len / size + 1) * 4;
	}
	else
		numVerts = numIndex = 4;
	...
	if (p->flags & PART_SPARK) // QMB style particles (8-sided cone)
	{
		float	angle;
		vec3_t	v;
		int		i, j, baseindex = rb_vertex;
		...
		for (i=1; i<9; i++) {
			...
			indexArray[rb_index++] = baseindex+j;
		}
	}
...
void R_RenderDecal (particle_t *p)
{
	...
	vec4_t	partColor;
	int		i, j, numVerts, numIndex, baseindex;
