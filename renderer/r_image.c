...
// Knightmare- default to trilinear filtering, thnx to MrG
// removed because this causes problems on some vidcards
...
qboolean GL_Upload32 (unsigned *data, int width, int height, qboolean mipmap)
{
	...
	// find sizes to scale to
	{
		...
		if (scaled_width > gl_config.max_texsize)
			scaled_width = gl_config.max_texsize;
		if (scaled_height > gl_config.max_texsize)
			scaled_height = gl_config.max_texsize;
	}
...
image_t	*R_FindImage (char *name, imagetype_t type)
{
	...
	/*	this is redundant because of jpg fallback
		// if no TGA, check for JPG
		s[len-3]='j'; s[len-2]='p'; s[len-1]='g';
		image = R_FindImage(s,type);
		if (image)
			return image;*/
...
#ifdef ROQ_SUPPORT
	r_rawtexture->registration_sequence = registration_sequence;
#endif // ROQ_SUPPORT

	r_envmappic->registration_sequence = registration_sequence;
	r_spheremappic->registration_sequence = registration_sequence;
	r_causticpic->registration_sequence = registration_sequence;
