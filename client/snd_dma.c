...
void S_Init (void)
{
	...
	#ifdef OGG_SUPPORT
		s_musicvolume = Cvar_Get ("s_musicvolume", "0.4", CVAR_ARCHIVE); // Q2E
	#endif
...
void S_AddLoopSounds (void)
{
	...
	for (i=0 ; i<cl.frame.num_entities ; i++)
	{
		...
		if (ent->solid == 31) // special value for bmodels
		{
			...
			S_SpatializeOrigin (origin_v, 255.0, SOUND_LOOPATTENUATE, // was ent->origin
		...
		{
			// find the total contribution of all sounds of this type
			S_SpatializeOrigin (ent->origin, 255.0, SOUND_LOOPATTENUATE, // was ent->origin
				&left_total, &right_total);
			for (j=i+1 ; j<cl.frame.num_entities ; j++)
			{
				if (sounds[j] != sounds[i])
...
void S_Update (vec3_t origin, vec3_t forward, vec3_t right, vec3_t up)
{
	...
	//if (!cl_paused->value) // Knightmare- stop music if paused
